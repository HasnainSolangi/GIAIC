...
class Customer {
  ...
  constructor(name: string, account: BankAccount) {
    this.name = name;
    this.account = account;
  }

  async performTransaction(action: string): Promise<void> {
    switch (action.toUpperCase()) {
      case "DEPOSIT":
        const { depositAmount } = await inquirer.prompt<{ depositAmount: string }>({
          type: "input",
          name: "depositAmount",
        ...
        break;
      case "WITHDRAW":
        const { withdrawAmount } = await inquirer.prompt<{ withdrawAmount: string }>({
          type: "input",
          name: "withdrawAmount",
...
async function main(): Promise<void> {
  const { customerName } = await inquirer.prompt<{ customerName: string }>({
    type: "input",
    name: "customerName",
    message: "Enter your name:",
  });

  const { initialBalance } = await inquirer.prompt<{ initialBalance: string }>({
    type: "input",
    name: "initialBalance",
  ...
  while (shouldContinue) {
    ...
    if (action.toUpperCase() === "EXIT") {
      ...
      const { continueTransaction } = await inquirer.prompt<{ continueTransaction: boolean }>({
        type: "confirm",
